...
sort_params = {
    "lowest_comparison_price": "&sort=comparisonPrice%3Aasc",
...
def get_searches(
    query: str, output_json: bool, print_results=False, sorting_parameters=""
) -> dict:
    ...
    element = WebDriverWait(driver=driver, timeout=5).until(
        EC.visibility_of_element_located(
            (By.CSS_SELECTOR, "article[data-test-id=product-card]")
        )
    )
...
def query_to_json(soup: BeautifulSoup) -> dict:
    ...
    for index, tag in enumerate(
        soup.select("article[data-test-id=product-card]", limit=5)
    ):
        products.update({index: {}})
        products[index].update(
            {
                "product_name": tag.select_one(
                    "a[data-test-id=product-card__productName]"
                ).text
            }
        )
        products[index].update(
            {
                "unit_price": tag.select_one(
                    "span[data-test-id=product-price__unitPrice]"
                ).text
            }
        )
        products[index].update(
            {
                "comparison_price": tag.select_one(
                    "div[data-test-id=product-card__productPrice__comparisonPrice]"
                ).text
            }
        )
